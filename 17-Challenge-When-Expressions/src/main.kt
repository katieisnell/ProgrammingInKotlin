 ...
 */
    var person = Pair("Bob", 22)

    when (person.second) {
        in 0..2 -> println("${person.first} is an Infant")
        in 3..12 -> println("${person.first} is a Child")
        in 13..19 -> println("${person.first} is a Teenager")
        in 20..39 -> println("${person.first} is an Adult")
        else -> println("${person.first} is Elderly")
    }
